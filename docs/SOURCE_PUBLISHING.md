...
首个 GitHub 版本固定为 `3.0.0`，标签为 `v3.0.0`。首次源码快照推送前的修改和本地提交不再递增版本；首次推送后移除 [AGENTS.md](../AGENTS.md) 中的临时冻结规则。
...
6. 使用 `git add -A` 暂存本次快照并检查 `git diff --cached`，提交为 `Release <version>`，再创建 `v<version>` 标签。没有内容变更时不创建重复提交；已有标签不能覆盖。
